...
class MultiTaskRobertaModel(nn.Module):
    def __init__(self, roberta_name, num_annots, num_classes):
        ...
        self.roberta = RobertaModel.from_pretrained(roberta_name,device_map = 'cuda:1')
        self.config = self.roberta.config
        self.classifiers = nn.ModuleList([nn.Linear(self.roberta.config.hidden_size, num_classes).to(device) for _ in range(num_annots)])

    def forward(self, input_ids, attention_mask, labels=[]):
        outputs = self.roberta(input_ids=input_ids, attention_mask=attention_mask)
        last_hidden_state = outputs.last_hidden_state[:, 0, :]  # Use the [CLS] token representation
        outputs = [classifier(last_hidden_state).to(device) for classifier in self.classifiers]
        return outputs
...
def main(filename, model_id, dataset_name, remove_columns, dataset_mode, target_col):
    ...
    for row in tokenized_dataset['test']:
        ...
        print(torch.tensor(row['input_ids']).to(device).unsqueeze(0).shape)
        print(torch.tensor(row['attention_mask']).to(device).unsqueeze(0).shape)
    ...
    print(p)
    print('=========================')
    print(len(p[0]), len(p[0][0]))
    print('=========================')
    print(p[1])
    print(len(p[1]), len(p[1][0]))
